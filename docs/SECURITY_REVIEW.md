...
| SEC-002 | OpenClaw capability and ingress | High | ADR 0013, R2 local TLS matrix, OpenClaw 2026.7.1 config/plugin/route-filter verifier | The R1 image accepts unrelated WebSocket paths through OpenClaw's generic Gateway fallback; Maritime also does not document WSS upgrades, subprotocol preservation, payload limits, idle timeouts, or stability. | Reject the R1 digest; publish only the tested exact-route filter with the general Gateway on loopback, then require the complete remote-extension phase matrix, private proxy evidence, plain/deep audits, per-user isolation, and route-only exposure. | Infrastructure | No for core; yes for browser experimental | Local repair passes; replacement image and live acceptance open |
