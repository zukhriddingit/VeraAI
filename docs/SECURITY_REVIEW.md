...
| SEC-013 | Self-service privacy lifecycle | Medium | authenticated privacy routes/UI, `0010_privacy_lifecycle.sql`, privacy repository/service, restore-time receipt CLI, and focused tests | The code now provides owner-only export, two-step deletion, provider/browser revocation, receipt-gated append-only cascades, backup-aging metadata, and restore reapplication. Live owner isolation and isolated restore behavior are not yet production evidence. | Run the live disposable-owner deletion and offline restored-backup rehearsal before enabling any nonfounder browser tester. Keep private receipts and outputs gitignored. | Privacy | No for founder staging; Yes for multi-user beta | Implemented; live rehearsal required |
...
- **SEC-013:** `apps/web/app/api/settings/privacy`, the authenticated Settings UI, migration
  `0010_privacy_lifecycle.sql`, and `PrivacyLifecycleRepository` implement owner-derived export and
  two-step deletion. PostgreSQL tests prove a receipt permits only the nested owner cascade while a
  direct append-only delete remains blocked and another owner remains intact. The restore CLI
  accepts only a mode-`0600` strict receipt ledger and an environment database whose name matches
  the operator confirmation; its unit tests cover unsafe arguments/files/receipts and count-only
  evidence. This remains **Implemented; live rehearsal required**, not resolved.
...
### Gateway replacement supply-chain evidence — 2026-07-26

- The public digest
  `sha256:5d1f6d2d097bb8e53f2e2dd6c1e6f8499d6daf34dff8a61b9b0c187fd9e1ec6b`
  was rejected after Trivy found 23 critical and 98 high vulnerabilities. It was not signed or
  deployed and is not a rollback artifact.
- The repair preserves the reviewed OpenClaw 2026.7.1 application image and source commit,
  replaces only five vulnerable application package instances using fixed integrity-locked
  tarballs, and transplants `/app` into the immutable zero-finding Chainguard Node `linux/amd64`
  base recorded in `remote-extension-runtime-lock.json`.
- The final image contains only `/usr/bin/node` as an executable. BusyBox, shell, npm, npx,
  node-gyp, Corepack, pnpm, and the package-manager library are absent. A fixed Node supervisor
  requires UID/GID `1000:1000`, the private state boundary, and the exact route-filter child.
- The local candidate passed the static supply-chain gate, all 1,128 unit tests, 28 focused
...
blockers; SEC-013 remains a multi-user blocker until its live deletion and isolated restore
rehearsals pass. Founder staging remains conditional on immutable
