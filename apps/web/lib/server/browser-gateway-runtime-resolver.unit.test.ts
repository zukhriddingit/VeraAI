...
const checkpointTokenDigest = createHash("sha256")
  .update(checkpointToken, "utf8")
  .digest("hex");
...
function fixture() {
  const getActiveForUser = vi.fn<
    (userId: VeraUserId) => Promise<BrowserGatewayAssignment | null>
  >(async () => assignmentA);
  const getActiveByCheckpointDigest = vi.fn<
    (digest: string) => Promise<BrowserGatewayAssignment | null>
  >(async () => assignmentA);
  const listEnabledConnectorIdsForUser = vi.fn<
    (userId: VeraUserId) => Promise<readonly string[]>
  >(async () => [
