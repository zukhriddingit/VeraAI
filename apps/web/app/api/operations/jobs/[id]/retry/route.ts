...
import { retrySourceJob } from "../../../../../../lib/server/operations-service.ts";
...
export async function POST(
  ...
  try {
    ...
    const queued = await retrySourceJob({
    ...
    const job = await dispatchHostedSourceJob(
      { userId: session.userId, repositories: session.repositories },
      queued.id
    );
    ...
    const code =
      error instanceof Error && /^[a-z_]+(?::[a-z_]+)?$/u.test(error.message)
        ? error.message
        : "invalid_request";
    return Response.json(
      { code },
