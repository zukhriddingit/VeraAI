import { CancelViewingRequestSchema, CancelViewingResponseSchema } from "@vera/domain";
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const input = CancelViewingRequestSchema.parse(await readCalendarMutationJson(request));
    const result = await service.cancel({ viewingId: id, ...input });
    return Response.json(CancelViewingResponseSchema.parse(result), {
      status: 200,
