...
describe("Vera Store extension verifier", () => {
  it("accepts the reviewed Store boundary", () =>
    expect(findVeraExtensionViolations(clean)).toEqual([]));
  it("rejects another readiness origin", () =>
    expect(
          ...
          content_scripts: [
            {
              ...
              matches: [...manifest.content_scripts[0].matches, "https://verahousing.app/*"]
            }
          ]
        }
      })
    ).toContain("Readiness bridge origins are not exact."));
  it("rejects added browser authority", () =>
    expect(
      findVeraExtensionViolations({
        ...clean,
  ...
  it("rejects an incorrectly sized icon", () =>
    expect(
      findVeraExtensionViolations({
        ...clean,
        iconDimensions: new Map([["icon-16.png", [32, 32]]])
      })
    ).toContain("Extension icons are missing or incorrectly sized."));
});
